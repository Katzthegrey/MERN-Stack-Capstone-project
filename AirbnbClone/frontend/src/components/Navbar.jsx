...
const Navbar = () => {
    ...
    const { navigate, token, username, logout } = useContext(AirbnbContext);
        ...
        <div className={`fixed top-0 left-0 right-0 z-50 flex flex-col py-3 font-medium border-b transition-colors duration-300 ${
            ...
            <div className='flex items-center justify-between px-4'>
                ...
                <div className='hidden md:flex items-center gap-6'>
                    <Link to={'/'} onClick={() => setActiveTab('places')}
                        ...
                        Places to stay
                        {activeTab === 'places' && <span className='absolute bottom-0 left-0 right-0 h-0.5 bg-[#FF385C] rounded-full'></span>}
                    </Link>
                    <Link to={'/experiences'} onClick={() => setActiveTab('experiences')}
                        className={`relative text-sm font-medium pb-1 transition ${
                        ...
                        Experiences
                        {activeTab === 'experiences' && <span className='absolute bottom-0 left-0 right-0 h-0.5 bg-[#FF385C] rounded-full'></span>}
                    </Link>
                </div>
                ...
                <div className='flex items-center gap-3'>
                    ...
                    <div className='relative' ref={hamburgerRef}>
                        <button onClick={() => setHamburgerDropdownOpen(!hamburgerDropdownOpen)}
                        ...
                        {hamburgerDropdownOpen && (
                            <div className={`absolute right-0 mt-2 w-56 border rounded-lg shadow-lg z-20 ${
                                ...
                                <button onClick={() => { toggleTheme(); setHamburgerDropdownOpen(false); }}
                    ...
                    <div className='relative' ref={userMenuRef}>
                        <button
                            ...
                            className={`w-9 h-9 rounded-full flex items-center justify-center border transition overflow-hidden ${
                                isDark ? 'border-gray-600 hover:bg-gray-800' : 'border-gray-300 hover:shadow-md'
                            }`}>
                            {token && username ? (
                                <span className={`text-sm font-semibold ${isDark ? 'bg-gray-600 text-white w-full h-full flex items-center justify-center' : 'bg-gray-800 text-white w-full h-full flex items-center justify-center'}`}>
                                    {getInitial()}
                                </span>
                            ) : (
            ...
            <div className='flex items-center justify-center mt-2'>
                <div className={`flex items-center border rounded-full shadow-sm hover:shadow-md transition ${
                    ...
                    <div className='relative' ref={provinceRef}>
                        <button onClick={() => { setProvinceDropdownOpen(!provinceDropdownOpen); setCheckInDropdownOpen(false); setCheckOutDropdownOpen(false); setGuestsDropdownOpen(false); }}
                            className={`flex flex-col items-start px-4 py-2 rounded-l-full min-w-[120px] transition ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
                        ...
                        {provinceDropdownOpen && (
                            <div className={`absolute top-full left-0 mt-2 w-56 border rounded-lg shadow-lg z-20 max-h-60 overflow-y-auto ${
                                ...
                                <div className='p-2'>
                                    <button onClick={() => { setSelectedProvince(''); setProvinceDropdownOpen(false); }}
                                        className={`block w-full text-left px-4 py-2 text-sm rounded-md transition mb-1 ${isDark ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-50'}`}>
                                        Anywhere
                                    </button>
                                    {PROVINCES.map((province) => (
                                        <button key={province} onClick={() => { setSelectedProvince(province); setProvinceDropdownOpen(false); }}
                                            className={`block w-full text-left px-4 py-2 text-sm rounded-md transition ${isDark ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-50'}`}>
                                            📍 {province}
                                        </button>
                                    ))}
                                </div>
                    ...
                    <div className='relative' ref={checkInRef}>
                        <button onClick={() => { setCheckInDropdownOpen(!checkInDropdownOpen); setProvinceDropdownOpen(false); setCheckOutDropdownOpen(false); setGuestsDropdownOpen(false); }}
                            className={`flex flex-col items-start px-4 py-2 min-w-[110px] transition ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
                        ...
                        {checkInDropdownOpen && (
                            <div className={`absolute top-full left-0 mt-2 border rounded-lg shadow-lg z-20 p-3 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                                <input type="date" value={checkIn} min={getToday()}
                                    onChange={(e) => { setCheckIn(e.target.value); setCheckInDropdownOpen(false); if (checkOut && new Date(checkOut) <= new Date(e.target.value)) setCheckOut(''); }}
                                    className={`border rounded-lg px-3 py-2 text-sm w-full ${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'}`} />
                            </div>
                        )}
                    </div>
                    ...
                    <div className='relative' ref={checkOutRef}>
                        <button onClick={() => { setCheckOutDropdownOpen(!checkOutDropdownOpen); setProvinceDropdownOpen(false); setCheckInDropdownOpen(false); setGuestsDropdownOpen(false); }}
                            className={`flex flex-col items-start px-4 py-2 min-w-[110px] transition ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
                        ...
                        {checkOutDropdownOpen && (
                            <div className={`absolute top-full left-0 mt-2 border rounded-lg shadow-lg z-20 p-3 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                                <input type="date" value={checkOut} min={getMinCheckOut()} disabled={!checkIn}
                                    onChange={(e) => { setCheckOut(e.target.value); setCheckOutDropdownOpen(false); }}
                                    className={`border rounded-lg px-3 py-2 text-sm w-full disabled:opacity-50 ${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'}`} />
                            </div>
                        )}
                    </div>
                    ...
                    <div className='relative flex items-center' ref={guestsRef}>
                        <button onClick={() => { setGuestsDropdownOpen(!guestsDropdownOpen); setProvinceDropdownOpen(false); setCheckInDropdownOpen(false); setCheckOutDropdownOpen(false); }}
                            className={`flex flex-col items-start px-4 py-2 min-w-[100px] transition ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
                        ...
                        <button onClick={handleSearch} disabled={!isSearchReady}
                            className={`p-2 rounded-full mr-1 transition ${isSearchReady ? 'bg-[#FF385C] text-white hover:bg-[#e0314f]' : (isDark ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-gray-100 text-gray-400 cursor-not-allowed')}`}>
                            <i className="fas fa-search text-sm"></i>
                        </button>
                        {guestsDropdownOpen && (
                            <div className={`absolute top-full right-0 mt-2 border rounded-lg shadow-lg z-20 p-3 w-56 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                                <div className='flex items-center justify-between'>
                                    <button onClick={() => setGuests(Math.max(1, guests - 1))}
                                        className={`w-8 h-8 rounded-full border flex items-center justify-center text-lg ${isDark ? 'border-gray-600 hover:bg-gray-700 text-white' : 'border-gray-300 hover:bg-gray-50'}`}>−</button>
                                    <span className={`text-lg font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{guests}</span>
                                    <button onClick={() => setGuests(Math.min(16, guests + 1))}
                                        className={`w-8 h-8 rounded-full border flex items-center justify-center text-lg ${isDark ? 'border-gray-600 hover:bg-gray-700 text-white' : 'border-gray-300 hover:bg-gray-50'}`}>+</button>
                                </div>
                                <button onClick={() => setGuestsDropdownOpen(false)}
                                    className={`w-full mt-3 py-1.5 rounded-lg text-sm font-medium transition ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Done</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {visible && <div className="fixed inset-0 bg-black bg-opacity-50 z-30" onClick={() => setVisible(false)} />}
        </div>
    );
};
